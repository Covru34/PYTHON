...
def searchAbonent():
    text = input("Що шукаємо: ")

    file = open("abonents.txt", "r", encoding="utf-8")
    ...
    while i < len(lines):
        # об'єднуємо всі 5 рядків одного абонента в один текст
        abonent = lines[i] + lines[i+1] + lines[i+2] + lines[i+3] + lines[i+4]

        # якщо введений текст є всередині абонента
        if text in abonent:
            print("----- ЗНАЙДЕНО -----")
